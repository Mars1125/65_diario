...
async function ordenarSenadores( $sexo ) {
    ...
    $.each( $partidos,  function( key, partido ) {
        ...
        $("#contSenadoras").append(' <div class="row hand btn-directorio mt-5 " data-bs-toggle="modal" data-bs-target="#directorioIntegrantes" data-fraccion="'+$nombre+'" id="scrollGo_'+$nombre+'" ><div class="col-md-1">'+$logo+'</div><div class="col-md-11 fs-3">  '+partido.nombre+' </div></div> ');
        $contCards = $('<div/>', {
        ...
        $.each( $senadorBuscar2, function( key, $senador ) {
            ...
            //         'src': '/senado/64_3/images/senadores/64/'+$senador.Foto,  
            //         'width': '100%'
            //         })
            // ); 
        ...
        if( $senador.url_sitio != '' && $senador.url_sitio != null  ) {
            ...
            $texto+='<i class="fa-solid fa-globe btn-reddit zoom" style="padding-top: 8px;width: 30px;height: 30px;text-align: center;"></i>';
            $texto+='</a>';
        }
        if($senador.facebook != '' && $senador.facebook != null ) {
            $texto+='<a href="'+$senador.facebook+'" target="_blank" class="" style="margin-left:4px;">';
            $texto+='<i class="fa-brands fa-facebook-f btn-facebook zoom" style="padding-top: 8px;width: 30px;height: 30px;text-align: center;"></i>';
            $texto+='</a> ';
        }
        if($senador.twitter != '' && $senador.twitter != null ) {
            $texto+='<a href="'+$senador.twitter+'" target="_blank" class="" style="margin-left:1px;">';
            $texto+='<i class="fa-brands fa-x-twitter btn-twitter zoom" style="padding-top: 8px;width: 30px;height: 30px;text-align: center;"></i>';
            $texto+='</a>';
        }
        if($senador.instagram != '' && $senador.instagram != null ) {
            $texto+='<a href="'+$senador.instagram+'" target="_blank" class="" style="margin-left:4px;">';
            $texto+='<i class="fa-brands fa-instagram btn-instagram zoom" style="padding-top: 8px;width: 30px;height: 30px;text-align: center;"></i>';
            $texto+='</a>';
        }
        if($senador.youtube != '' && $senador.youtube != null ) {
            $texto+='<a href="'+$senador.youtube+'" target="_blank" class="" style="margin-left:4px;">';
            $texto+='<i class="fa-brands fa-youtube btn-youtube zoom" style="padding-top: 8px;width: 30px;height: 30px;text-align: center;"></i>';
            $texto+='</a>';
        }   
...
function tblTotalesPartidos($partidos){
    ...
    $.each( $partidos, async function( key, partido ) {
        ...
        $th = $(`<tr class="borderb${$logo}" onclick="scrollGo( '${$logo}' )">
            <td align="left" style="vertical-align: middle;" width="110">
                <a href="#" >
