...
!function(t) {
    ...
    Raphael.el.trigger = function(t) {
        for (var o = 0, e = this.events.length; o < e; o++)
    ...
    t.fn.JSMaps = function(a) {
        ...
        function n(o) {
            for (var e, i = 0, a = o.length; i < a; i++)
                e = o[i],
                t.each(D.groups, function(o, i) {
                    t.each(i.members, function(t, o) {
                        e.name === o && (e.group = i.name,
                        e.color = i.color,
                        e.hoverColor = i.hoverColor,
                        e.selectedColor = i.selectedColor)
                    })
                });
            return o
        }
        ...
        function l() {
            function o() {
                P = d.parent().width(),
                w = x.width(),
                "text" === D.stateClickAction ? "bottom" === (h = w >= 767 ? D.textPosition : "bottom") ? (Z = (Y = P) / F,
                d.css({
                    width: Y + "px",
            ...
            function a(o) {
                i && e || D.disableTooltip || (p(),
                T.after(t("<div />").addClass("jsmaps-tooltip")),
                t(".jsmaps-tooltip").html(o),
                l(),
                t(".jsmaps-tooltip").fadeIn())
            }
            function l() {
                j = -40,
                k = v - t(".jsmaps-tooltip").height() + j < 0,
                j = k ? 40 : j - t(".jsmaps-tooltip").height(),
                t(".jsmaps-tooltip").css({
            ...
            function O(o) {
                ...
                if (o && o.pageX ? (f = o.pageX,
                v = o.pageY - e) : (f = event.clientX + document.body.scrollLeft,
                ...
                D.displayMousePosition) {
                    ...
                    t(".jsmaps-mouse-position .xPos").text("X: " + a),
                    t(".jsmaps-mouse-position .yPos").text("Y: " + s),
                    t(".jsmaps-mouse-position .mapXPos").text("Map X: " + n),
                    t(".jsmaps-mouse-position .mapYPos").text("Map Y: " + r)
                }
            }
            var Y = D.mapWidth
              , Z = D.mapHeight
            ...
            function() {
                ...
                for (var x = 0, w = I.length; x < w; x++) {
                    ...
                    i = u.path(I[x].path).attr(h),
                    A.push(i),
                    (I[x].enable && D.displayAbbreviations || !I[x].enable && D.displayAbbreviationOnDisabledStates) && (D.autoPositionAbbreviations ? (d = (l = i.getBBox()).x + l.width / 2 + I[x].textX,
                    m = l.y + l.height / 2 + I[x].textY) : (d = I[x].textX,
                    m = I[x].textY),
                    C.push(u.text(d, m, I[x].abbreviation).attr(f)));
                    var j, k, P;
                    I[x].group ? ((k = (j = D.groups.find(function(t, o) {
                        if (I[x].group === t.name)
                            ...
                            t.groupIds.push(x),
                            t
                    })).set.push(u.path(I[x].path)).attr(v)).data("group", j),
                    k.data("id", P),
                    k.name = j.name,
                    k.enable = j.enable) : ((k = u.path(I[x].path).attr(v)).data("id", x),
                    k.name = I[x].name,
                    k.enable = I[x].enable),
                    y.push(k),
                    k.mouseover(e),
                    ...
                    k.click(function(o) {
                        if (!S || !S.isDragging()) {
                            var e, i = this.data("id"), a = !!this.data("group"), n = a ? this.data("group") : I[i];
                            n.enable && (r(),
                            g && g != n && (e = g.groupIds || [g.id],
                            g.color,
                            s(b ? W : A, e, g.color)),
                            b = !1,
                            n != g && (e = a ? this.data("group").groupIds : [i],
                            s(A, e, n.selectedColor),
                            E && E.val(n.name)),
                            g = n,
                            "text" === D.stateClickAction ? M.html(n.text) : "url" === D.stateClickAction && window.open(n.url, D.hrefTarget)),
                            t.isFunction(c.onStateClick) && c.onStateClick.call(this, n)
                        }
                    })
                }
                D.displayMousePosition || (o(),
                u.setViewBox(0, 0, D.mapWidth, D.mapHeight),
                D.responsive && t(window).on("resize", function() {
                    o()
                }))
            }(),
            function() {
            ...
            X && X.length && function() {
                ...
                for (var e = 0; e < X.length; e++) {
                    ...
                    if (X[e].src && "" !== X[e].src) {
                        ...
                        i = u.circle(X[e].xPos, X[e].yPos, X[e].pinWidth || D.pinSize).attr(n);
                    i.data("id", e),
                    i.name = X[e].name,
                    i.enable = X[e].enable,
                    W.push(i),
                    ...
                    i.click(function(o) {
                        if (!S || !S.isDragging()) {
                            var e = this.data("id")
                              , i = X[e];
                            r(),
                            g && (pathIds = g.groupIds || [g.id],
                            s(b ? W : A, pathIds, g.color)),
                            b = !0,
                            s(W, [e], i.selectedColor),
                            g = i,
                            "text" === D.stateClickAction ? M.html(i.text) : "url" === D.stateClickAction && window.open(i.url, D.hrefTarget),
                            t.isFunction(c.onStateClick) && c.onStateClick.call(this, i)
                        }
                    })
                }
            }(),
            D.enablePanZoom && !D.displayMousePosition && function() {
                t("body").on("mousewheel DOMMouseScroll", function(o) {
                    t(o.target).parents(".jsmaps").length && (o.preventDefault(),
                    o.stopPropagation())
                }),
                (S = u.panzoom({
            ...
            D.selectElement && function() {
                ...
                D.selectElementDevices && D.selectElementDevices.length ? o.addClass(D.selectElementDevices.join(" ")) : o.addClass("all-devices"),
                (E = o.find("select")).find('option[value="default"]').text(D.selectElementDefaultText);
            ...
            m && m.length && m.fadeOut(),
            d.fadeIn()
        }
        function p() {
            u.remove(),
            d.find(".jsmaps-console").length && d.find(".jsmaps-console").remove(),
            d.find(".jsmaps-mouse-position").length && d.find(".jsmaps-mouse-position").remove(),
            d.find(".jsmaps-text").length && d.find(".jsmaps-text").remove(),
            d.siblings(".jsmaps-select").length && d.siblings(".jsmaps-select").remove(),
            S && (S = null),
            A = [],
            y = [],
            C = [],
        ...
        if (!window.JSMaps.maps[c.map])
            throw new Error("The data for " + c.map + " is missing");
        var d = t(this);
        if (c.displayPreloader)
            var m = t('<div class="jsmaps-preloader">' + c.preloaderText + "</div>").appendTo(d);
        var u, h, f = 0, v = 0, g = null, b = !1, x = t(window), w = x.width(), j = 0, k = !1, y = [], C = [], P = d.parent().width(), T = t('<div class="jsmaps"></div>').appendTo(d), z = "jsmaps-" + o();
        T.attr("id", z);
        var M, S, A = [], W = [], E = null, O = window.JSMaps.maps[c.map], D = t.extend(c, O.config), I = O.paths, X = O.pins;
        l(),
        d.on("stateClick", function(o, e) {
            t.each(y, function(t, o) {
                pathName = o.name,
                e === pathName && ("set" === y[t].type ? y[t][0] : y[t]).trigger("click")
            })
        }),
        d.on("stateUnClick", function() {
            if (g) {
                ...
                s(b ? W : A, t, g.color),
                "text" === D.stateClickAction && M.html(D.defaultText),
                g = null
            }
        }),
        d.on("reDraw", function(o, e) {
            e && (e.config && (D = t.extend(D, e.config)),
            S && D.retainPanZoomOnRedraw && (D.initialZoom = S.getCurrentZoom(),
            D.initialMapX = S.getCurrentPosition().x,
            D.initialMapY = S.getCurrentPosition().y),
            e.pins && (X = e.pins),
            e.paths && (I = e.paths),
            m && m.length ? m.fadeIn("fast", p) : p())
        }),
        c.onReady.call(this)
    }
}(jQuery);