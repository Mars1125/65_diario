...
function cabecera_diario($var2,$conecta_base) {
		$sql = "SELECT * FROM d_debates.diario INNER JOIN d_debates.contenido_diario ON diario.id = contenido_diario.id_diario WHERE contenido_diario.id='".$var2."'";		
...
function contenidoDiario($id,$conecta_base) {
		$sql = $consulta_base= "SELECT * FROM d_debates.contenido_diario WHERE id='$id' AND visible='1' ORDER BY contenido_diario.id ASC";
		...
		if($result->num_rows > 0) {
			...
			while ($row = $result->fetch_assoc()) {
				if(busca_link_asunto($row['id'], $conecta_base)==2){						
					...
					$texto = str_replace('" src="content/sp/dd/content/cale/diarios', '" src="http://infosen.senado.gob.mx/content/sp/dd/content/cale/diarios', $row['texto']);
